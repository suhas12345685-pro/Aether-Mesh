...
function buildGatewayServerMethodsEntries(): Record<string, string> {
  const methodsRoot = path.join(process.cwd(), "src", "gateway", "server-methods");
  const entries: Record<string, string> = {};
  ...
  // Also include the server-methods registry aggregator itself
  entries["gateway/server-methods"] = "src/gateway/server-methods.ts";
  entries["gateway/server-aux-handlers"] = "src/gateway/server-aux-handlers.ts";

  for (const dirent of fs.readdirSync(methodsRoot, { withFileTypes: true })) {
    ...
    if (
      ...
      dirent.name.includes("test-helpers") ||
      dirent.name.includes("test-support")
    ) {
      continue;
    }

    const nameWithoutExt = dirent.name.slice(0, -3); // remove ".ts"
    entries[`gateway/server-methods/${nameWithoutExt}`] = `src/gateway/server-methods/${dirent.name}`;
  }
...
function buildCoreDistEntries(): Record<string, string> {
  return {
    ...
    ...buildGatewayServerMethodsEntries(),
  };
}
